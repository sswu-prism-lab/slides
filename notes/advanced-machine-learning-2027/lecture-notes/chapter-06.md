...
그림 6.1 · (왼쪽) 세 변수 $a,b,c$의 방향성 그래프. (오른쪽) 완전 연결이 아닌 방향성 비순환 그래프 $x_1,\ldots,x_7$의 예.
...
그림 6.3 · (왼쪽) 관측 변수 $\{t_n\}$에 음영을 더한 모델. (오른쪽) 새 입력 $\widehat{x}$와 예측 $\widehat{t}$를 추가한 모델.
...
그림 6.4 · (왼쪽·가운데) 두 이산 변수의 완전 연결($K^2-1$)과 링크 제거($2(K-1)$). (오른쪽) $M$개 노드의 사슬은 $K-1+(M-1)K(K-1)$개로 $M$에 선형이다.
...
그림 6.5 · (왼쪽) 디리클레 사전 분포 추가, (가운데) 매개변수 매듭, (오른쪽) $M$개 부모와 단일 자식 $y$.
...
그림 6.6 · 링크 하나($x_1\to x_3$)가 빠진 가우시안 변수들의 방향성 그래프.
...
그림 6.7 · 꼬리 대 꼬리 노드 $c$. (왼쪽) 미관측 → 종속, (오른쪽) 관측 → 조건부 독립.
...
그림 6.8 · 머리 대 꼬리 노드 $c$. (왼쪽) 미관측 → 종속, (오른쪽) 관측 → 조건부 독립.
...
그림 6.9 · 머리 대 머리 노드 $c$. (왼쪽) 미관측 → 독립, (오른쪽) 관측 → 종속(설명해내기).
...
그림 6.12 · $A$에서 $B$로의 모든 경로가 $C$를 지나므로 $A\perp\!\!\!\perp B\mid C$가 성립한다.
...
그림 6.13 · 베이즈 정리의 그래프 표현. (왼쪽) $p(x,y)$, (가운데) $y$ 관측, (오른쪽) $p(y)p(x\mid y)$.
...
그림 6.14 · (왼쪽) 비방향성 사슬. (오른쪽) 양 끝에서 전방·후방 메시지를 전달해 $p(x_n)=\mu_\alpha(x_n)\mu_\beta(x_n)/Z$를 구한다.
