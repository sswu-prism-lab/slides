...
클래스 내 산포 행렬 $\mathbf{S}_\mathrm{W}$와 투영 벡터 $\mathbf{w}\propto\mathbf{S}_\mathrm{W}^{-1}(\mathbf{m}_2-\mathbf{m}_1)$을 계산합니다.
...
print(f"정규화 w/||w|| = {w / np.linalg.norm(w)}")
```

</PyRunner>

👉 [문제 5로 돌아가기](./problem-5)
