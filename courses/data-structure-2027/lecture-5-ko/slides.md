...
- 이러한 정의들과 연관되어 리스트 ADT에 대한 연산들의 집합이 존재한다:
  - `printList`와 `makeEmpty`는 이름 그대로의 명백한 일을 수행한다.
  - `find`는 어떤 항목이 처음으로 나타나는 위치를 반환한다.
  - `insert`와 `remove`는 특정 위치에 원소를 삽입하거나 제거한다.
  - `findKth`는 특정 위치의 원소를 반환한다.

- 어떤 함수가 적절한지에 대한 해석과 특이한 경우의 처리는 전적으로 프로그래머에게 달려 있다(예를 들어 `next`와 `previous`를 추가할 수도 있다).
...
- [스택]{.hl}은 삽입과 삭제가 오직 한 위치 — *리스트의 가장 끝*, 즉 [탑]{.hl}이라 불리는 위치 — 에서만 수행되도록 제한된 리스트이다.
  - 후입선출(Last-In-First-Out, LIFO) 특성.

- 기본 연산은 삽입에 해당하는 `push`와 가장 최근에 삽입된 원소를 삭제하는 `pop`이다.

- 가장 최근에 삽입된 원소는 `pop` 전에 `top` 연산을 통해 확인할 수 있다.
...
- [큐]{.hl} 역시 리스트이지만, 삽입은 한쪽 끝에서 이루어지고 삭제는 *다른 쪽 끝*, 즉 [프론트]{.hl}라 불리는 위치에서 수행된다.
  - 선입선출(First-In-First-Out, FIFO) 특성.

- 기본 연산은 삽입에 해당하는 `push`와 가장 먼저 삽입된 원소를 삭제하는 `pop`이다.

- 가장 먼저 삽입된 원소는 `pop` 전에 `front` 연산을 통해 확인할 수 있다.
...
- 이 두 변형 큐를 이용하면 다양한 문제에 대해 효율적이고 효과적인 알고리즘을 설계할 수 있다.
  - 우리가 배울 자료구조들은 컴퓨터 과학에서 가장 *근사한* 것들에 속한다.

- 이번 강의에서는 이 두 자료구조를 다루고 구현한다:
...
- 배열 기반 큐 구현에는 약점이 있다. `pop` 연산으로 원소를 제거하면 사용하지 않는 공간이 남게 된다.

- [원형 큐]{.hl}(환형 큐, 또는 *링 버퍼*라고도 불림)는 고정된 크기의 단일 버퍼를 그 양 끝이 연결된 것처럼 사용한다.

- 이는 연산이 *선입선출* 원칙을 따르되 저장 공간을 효율적으로 활용하는 원형 배열을 나타낸다.

- 이 큐의 크기는 고정되어 있으며, 구현의 첫 단계에서 결정된다.
...
- 큐의 시작과 끝이 원형으로 묶여 하나의 원을 이룬다.

- 원형 큐는 일반적으로 `front`와 `rear` 두 개의 포인터를 사용하여 첫째 원소와 마지막 원소를 추적한다.
  - `front` 포인터는 큐의 *시작*을 가리킨다.
  - `rear` 포인터는 큐의 *끝*을 가리킨다.
...
- 구현에는 큐 역할을 하는 고정 크기 배열과 두 개의 포인터 `front`, `rear`가 사용된다.
  - 이 두 포인터는 `-1`과 같은 특수한 값으로 초기화된다.

- 배열은 끝에 도달하면 다시 처음으로 *감싸 돌아간다* — 이것이 *원형*이라는 이름의 유래이다.

- 포인터 값을 이용하면 큐가 비어 있는지 혹은 가득 차 있는지를 확인할 수 있어, [오버플로우]{.hl}와 [언더플로우]{.hl}를 모두 쉽게 처리할 수 있다.

- 원형 큐는 원소가 dequeue된 공간을 재사용하여 선형 큐의 공간 낭비 문제를 없앤다.

- 삽입과 제거 연산에 대해 [$\mathcal{O}(1)$]{.hl} 시간복잡도를 제공한다.
...
public:
    ...
    void enqueue(int x) {              // rear 위치에 push
        if (isFull()) return;          // 오버플로우
        if (isEmpty()) front = 0;
        rear = (rear + 1) % cap;       // 감싸 돌아감
        arr[rear] = x;
    }
private:
    vector<int> arr;
...
```cpp
int dequeue() {                    // front에서 pop
    if (isEmpty()) return -1;      // 언더플로우
    int x = arr[front];
    if (front == rear)             // 마지막 원소였다면
        front = rear = -1;         // 비어있는 상태로 초기화
    else
        front = (front + 1) % cap; // 감싸 돌아감
    return x;
}

int getFront() const {             // 제거 없이 확인만
...
int main() {
    CircularQueue q(4);              // 용량 4, 최대 3개 저장
    for (int v : {10, 20, 30}) q.enqueue(v);
    cout << "dequeue -> " << q.dequeue() << "\n";   // 10이 빠짐
    q.enqueue(40);                   // 비워진 슬롯 재사용 (감싸 돌아감)
...
- [양방향 큐]{.hl}는 흔히 [덱]{.hl}(*"deck"*으로 발음)이라 불리며, 프론트와 리어 양 끝 모두에서 삽입과 제거를 허용하도록 큐를 일반화한 자료구조이다.

- 리어에서 삽입하고 프론트에서 제거하는 단순한 큐와 달리, 덱은 `enqueueFront`, `enqueueRear`, `dequeueFront`, `dequeueRear`라는 네 가지 주요 연산을 지원한다.

- 이러한 유연함 덕분에 덱은 일반 큐가 너무 제한적인 다양한 응용에 적합하다.
...
- 원형 큐 구현에서 출발하여 `enqueue`와 `dequeue`를 적절히 변형하라.

- `enqueueFront`와 `dequeueRear`를 추가하라.
  - 처음과 마지막 원소를 간단하게 확인할 수 있다.

- 테스트 코드를 추적하며 출력되는 값을 예측하라.
...
- [우선순위 큐]{.hl}는 적어도 두 가지 연산을 허용한다. 이름 그대로의 일을 수행하는 [`insert`]{.hl}, 그리고 최소값 원소를 찾아 반환하고 제거하는 [`removeMin`]{.hl}이다.
  - `insert`는 큐의 `push`에 해당한다.
  - `removeMin`은 큐의 `pop`에 해당하는 우선순위 큐의 연산이다.

- 대부분의 자료구조와 마찬가지로 다른 연산을 추가할 수도 있지만, 이는 확장일 뿐 기본 모델의 일부는 아니다.
...
- 우선순위 큐를 구현하는 몇 가지 명백한 방법이 있다.
  - 단순 *연결 리스트*: 프론트에 삽입하는 데는 $\mathcal{O}(1)$이지만, 최소값을 삭제하려면 리스트를 순회하는 $\mathcal{O}(N)$이 든다.
  - *정렬된 리스트*: `insert`는 비싸지만($\mathcal{O}(N)$) `removeMin`은 저렴하다($\mathcal{O}(1)$).

- 또 다른 방법은 [이진 탐색 트리]{.hl}를 사용하는 것이다.
  - 이 경우 두 연산 모두 평균적으로 $\mathcal{O}(\log N)$의 실행 시간을 가진다.
  - 삽입은 무작위이고 제거는 그렇지 않더라도 성립한다.
...
- *왼쪽* 서브트리의 노드를 반복적으로 제거하면 오른쪽 서브트리가 무거워져 트리의 균형이 깨질 것처럼 보인다.
  - 최악의 경우, `removeMin`이 왼쪽 서브트리를 소진시키고 나면 오른쪽 서브트리는 원래 있어야 할 것보다 최대 2배 많은 원소를 가진다.

- 탐색 트리를 사용하는 것은 과잉일 수 있다 — 필요하지 않은 많은 연산을 지원하기 때문이다.

- 우리는 *어떠한 링크도 없이* 두 연산 모두 최악의 경우에도 [$\mathcal{O}(\log N)$]{.hl} 시간으로 우선순위 큐를 구현할 수 있다(삽입은 평균적으로 상수 시간이 든다).
...
- 우리가 사용할 구현은 [이진 힙]{.hl}으로 알려져 있으며, 이를 단순히 *힙*이라 부른다.

- 우선순위 큐 구현에 매우 흔히 쓰이기 때문에, 이 맥락에서는 별다른 수식어 없는 *힙*이라는 단어가 일반적으로 이 자료구조를 가리킨다.

- 이진 탐색 트리처럼, 힙도 [구조 성질]{.hl}과 [힙-순서 성질]{.hl} 두 가지 성질을 가진다.

- 힙에 대한 연산은 이 성질들 중 하나를 깨뜨릴 수 있으므로, 힙 연산은 모든 힙 성질이 다시 올바르게 유지될 때까지 종료되어서는 안 된다.
...
- 힙은 *완전히 채워진* 이진 트리이되, 가장 아래 레벨만은 예외로 왼쪽에서 오른쪽으로 채워진다.
  - 이 규칙이 [구조 성질]{.hl}이다.
  - 이러한 트리를 [완전 이진 트리]{.hl}라고 한다.

- 높이 $h$의 완전 이진 트리는 $2^h$개에서 $2^{h+1}-1$개 사이의 노드를 가진다.
  - 이는 높이가 [$\mathcal{O}(\log N)$]{.hl}임을 의미한다.
...
- 완전 이진 트리는 매우 규칙적이기 때문에 [배열]{.hl}로 표현될 수 있으며 링크가 필요하지 않다. ($0$번째 원소는 비워둔다는 점에 유의하라.)
  - 배열 위치 $i$의 임의의 원소에 대해, 왼쪽 자식은 $2i$에, 오른쪽 자식은 바로 다음 칸인 $2i+1$에 있으며, 부모는 $\lfloor i/2 \rfloor$에 있다.

- 트리를 순회하는 연산은 극히 단순하며 매우 빠르게 수행될 가능성이 높다.
  - 유일한 단점은 최대 힙 크기를 미리 추정해야 한다는 것이지만, 보통 이는 문제가 되지 않는다.

- 힙은 하나의 배열과 현재 힙 크기를 나타내는 정수로 구성된다.
...
- 연산을 빠르게 수행할 수 있게 하는 성질이 [힙-순서 성질]{.hl}이다.

- 최소값을 빠르게 찾고자 하므로, 가장 작은 원소가 *루트*에 있어야 한다.
  - 모든 서브트리 또한 힙이라면, 임의의 노드는 그 모든 자손보다 작다.

- 힙-순서 성질: 모든 노드 $X$에 대해, $X$의 부모의 키는 $X$의 키보다 작다(또는 같다). 단, 루트는 예외이다.
...
- 원소 $X$를 삽입하기 위해, 다음으로 가능한 위치에 *공간*(hole)을 만든다(그렇지 않으면 트리가 완전하지 않게 된다).
  - $X$를 힙 순서를 위반하지 않고 공간에 넣을 수 있다면 그대로 넣고 끝낸다.
  - 그렇지 않으면 공간의 부모에 있는 원소를 공간으로 내려 공간을 루트 방향으로 올려보내며, $X$를 넣을 수 있을 때까지 계속한다.

- 이 일반적인 전략을 [스며오르기]{.hl}(percolate up)라고 하며, 새 원소가 올바른 위치를 찾을 때까지 힙 위로 스며오른다.

- 스며오르기를 반복적인 *교환*으로 구현할 수도 있지만, 교환은 여러 번의 할당이 필요하므로 밀어내는 방식이 더 저렴하다.

- 새 원소가 최소값이어서 루트까지 끝까지 스며오르는 경우, 삽입에는 최대 [$\mathcal{O}(\log N)$]{.hl} 시간이 소요될 수 있다.
...
$14$를 삽입하기: 공간이 마지막 위치에서 위로 스며오르며, $14$가 부모($13$)가 더 작은 자리를 찾을 때까지 올라간다.
...
- `removeMin` 연산은 삽입과 거의 같은 방식으로 처리된다.

- 최소값이 제거되면 *루트*에 공간이 생긴다. 힙이 하나 작아지므로 마지막 원소 $X$는 힙 안 어딘가로 옮겨져야 한다.
  - $X$를 그 공간에 넣을 수 있다면 끝난다.
  - 그럴 가능성은 낮으므로, 공간의 자식 중 *더 작은* 쪽을 공간으로 올려 공간을 한 레벨 아래로 밀어내며, $X$가 들어맞을 때까지 반복한다.

- 이 일반적인 전략을 [스며내리기]{.hl}(percolate down)라고 한다.

- 이 연산의 최악의 경우 실행 시간은 [$\mathcal{O}(\log N)$]{.hl}이다.
...
$13$을 제거하기: 마지막 원소($31$)가 루트의 공간을 채운 뒤, 각 레벨에서 더 작은 자식을 지나 아래로 스며내려가며 힙 순서가 복원된다.
...
public:
    ...
    void insert(int x) {                 // O(log N)
        ...
        array[0] = x;                    // 감시자가 루프를 멈춤
        for (; x < array[hole / 2]; hole /= 2)
            array[hole] = array[hole / 2];   // 부모를 아래로 밀어냄
        array[hole] = x;                 // x를 배치
    }
private:
    vector<int> array;
...
void deleteMin() {                       // O(log N)
    if (isEmpty()) return;
    array[1] = array[currentSize--];     // 마지막을 루트로 옮김
    percolateDown(1);
}
void buildHeap() {                       // O(N): 제자리에서 힙화
...
- 운영체제에서 우선순위 큐는 프로세스와 작업을 그 [우선순위]{.hl}에 따라 관리하는 효과적인 방법이다.
  - 주된 목적은 높은 우선순위의 작업이 낮은 우선순위의 작업보다 먼저 실행되도록 보장하여 성능과 응답성을 최적화하는 것이다.

- 대표적인 응용 중 하나는 프로세스의 [스케줄링]{.hl}이다.
  - [우선순위 스케줄링]{.hl}에서는 각 프로세스에 우선순위 레벨이 할당되고, 스케줄러는 이 레벨에 따라 우선순위 큐에서 프로세스를 선택한다.

- 이 방식은 *선제적*(preemptive)일 수도 있고 *비선제적*(non-preemptive)일 수도 있다:

<div class="sub-item-enum">

1. *선제적*: 실행 중인 프로세스보다 높은 우선순위의 새 프로세스가 도착하면, 현재 프로세스는 선점(일시 중단)되어 큐에 다시 추가되고, 새 프로세스가 먼저 실행된다.
2. *비선제적*: 더 높은 우선순위의 프로세스가 도착하더라도 실행 중인 프로세스가 완료되고, 새 프로세스는 그다음에 실행된다.
