...
- 집합 $S$에서 모든 쌍 $(a, b)$ ($a, b \in S$)에 대해 명제 $a\ R\ b$가 *참*이거나 *거짓*으로 결정되면, [관계]{.hl} $R$이 $S$ 위에 정의되었다고 한다.
  - $a\ R\ b$가 참이면, $a$가 $b$와 *관계되었다*고 한다.

- [동치 관계]{.hl}는 다음 세 가지 성질을 만족하는 관계 $R$이다:

<div class="sub-item-enum">

1. *재귀성.* $a\ R\ a$, $\forall a \in S$.
2. *대칭성.* $a\ R\ b$이면 그리고 오직 그때에만 $b\ R\ a$이다.
...
- `find` 연산은 배열에서 $\mathcal{O}(N)$, 자가-조정 이진 트리에서 $\mathcal{O}(\log N)$의 시간을 소모한다.
  - 원소를 $\mathcal{O}(1)$ 시간에 `find`할 수 있을까?

- [해시 테이블]{.hl}의 구현은 흔히 [해싱]{.hl}이라 불리며, 삽입·삭제·탐색을 *평균 상수 시간*에 수행하는 기법이다.

- 원소들 사이의 *순서*를 요구하는 연산은 효율적으로 지원되지 않는다. 예를 들어 `findMin`, `findMax`, `print`를 선형 시간에 수행하는 것이 그러하다.

- 오늘은 해시 테이블 자료구조를 다룬다:
...
<div>
    ...
    <p style="margin: 2rem 0; font-size: 1.3rem;"><span style="color:#DBE3EA; font-weight:bold;">최악의 경우 O(1) 접근을 가지는 해시 테이블</span></p>
  </div>
...
- 이상적인 해시 테이블 자료구조는 단지 원소들을 담는 고정된 크기의 *배열*이다.

- 보통 탐색은 원소의 일부분에 대해 수행되며, 이 부분을 [키]{.hl}라고 한다.

- 각 키는 `0`부터 `tableSize - 1` 범위의 어떤 숫자로 대응되어 적절한 위치(cell)에 놓인다.
  - 이 대응을 [해시 함수]{.hl}라 하며, 이상적으로는 계산이 간단해야 하고 서로 다른 두 키가 서로 다른 위치를 갖도록 보장해야 한다.

- 위치는 유한하지만 키는 사실상 무수히 많으므로 이는 명백히 불가능하다. 따라서 키들을 위치에 *고르게 분산시키는* 해시 함수를 찾고자 한다.
...
- 오른쪽 테이블은 전형적인 *완벽한* 상황이다 — 모든 키가 각자 자기 위치에 놓인다.

- 이것이 해싱의 기본 아이디어이다.

- 남은 문제는 함수를 선택하는 것, 두 키가 같은 값으로 해시될 때 어떻게 할지 정하는 것, 그리고 테이블 크기를 정하는 것이다.
  - 두 키가 같은 값으로 해시되는 것을 [충돌]{.hl}이라 한다.
...
- 키가 정수라면, 키에 바람직하지 않은 성질이 없는 한 `key mod tableSize`를 반환하는 것이 일반적으로 타당하다.
  - 테이블 크기가 $10$이고 모든 키가 0으로 끝난다면, 표준 해시 함수는 나쁜 선택이다.
  - 이런 상황을 피하기 위해, 테이블 크기를 [소수]{.hl}로 두는 것이 흔히 좋은 방법이다.

- *임의의* 정수 키에 대해 이 함수는 계산이 간단하고 키를 고르게 분산시킨다.
  - 키가 문자열일 때는 문자들의 ASCII 값을 결합할 수 있다.

- 테이블 크기가 크면, 단순히 더하는 방식은 잘 분산되지 않는다.
  - 테이블 크기가 $10007$(소수)이고 키가 8자 이하일 때, 합은 $0$부터 $1016$까지밖에 도달하지 못한다 (ASCII 문자는 최대 $127$이고, $127 \times 8 = 1016$이다).
...
- 두 번째 시도는 앞의 세 문자에 가중치를 준다: $\texttt{key}[0] + 27\,\texttt{key}[1] + 729\,\texttt{key}[2]$.

- 안타깝게도 영어는 무작위가 *아니다*.
  - $26^3$개의 세 문자 조합 중 큰 사전에서는 오직 $2851$개의 서로 다른 조합만 나타난다 — 따라서 충돌이 전혀 없더라도 테이블의 약 $28\%$만 도달할 수 있다.
...
- 세 번째 시도는 키의 *모든* 문자를 포함하며 일반적으로 잘 분산될 것이라 기대할 수 있다.

- 이 코드는 키를 $37$진수로 취급하여, [호너의 규칙]{.hl}을 사용해 다항 함수를 계산한다.

- 대단히 단순하다는 장점이 있으며 상당히 빠르다.
...
- 남은 주요 구현 세부사항은 [충돌 해결]{.hl}이다.

- 삽입되는 원소가 이미 삽입된 원소와 같은 값으로 해시되면 *충돌*이 발생하며, 이를 해결해야 한다.

- 첫 번째 전략인 [개별 체이닝]{.hl}은 같은 값으로 해시되는 모든 원소를 *리스트*로 유지한다.

- $hash(x) = x \bmod 10$로 $\{0,1,4,9,16,25,36,49,64,81\}$를 삽입하면 오른쪽 테이블을 얻는다.
...
- `find`를 수행하려면, 해시 함수를 사용해 순회할 리스트를 정한 뒤 그 리스트를 탐색한다.

- `insert`를 수행하려면, 적절한 리스트를 확인해 원소가 이미 있는지 살펴본다.
  - 원소가 새로운 것이라면 리스트의 *앞*에 삽입할 수 있다.

- 연결 리스트 외에 어떤 방식이든 사용할 수 있다 — 이진 탐색 트리나 심지어 또 다른 해시 테이블도 가능하다.
  - 그러나 테이블이 크고 해시 함수가 좋다면 모든 리스트가 짧으므로, 기본적인 개별 체이닝이 단순하게 유지된다.

- 해시 테이블의 [적재율]{.hl} $\lambda$는 원소 수와 테이블 크기의 비율이다.
...
public:
    ...
    bool insert(const HashedObj& x) {
        ...
        if (++currentSize > theLists.size()) rehash();   // 적재율 lambda <= 1 유지
        return true;
    }
...
- 개별 체이닝은 연결 리스트를 사용한다는 단점이 있다 — 새 위치를 할당하는 것은 느리고 두 번째 자료구조가 필요하다.

- 다른 대안은 연결 리스트 *없이* 빈 위치를 찾을 때까지 대체 위치를 시도하여 충돌을 해결한다.
  - 이 전략을 [개방 주소]{.hl}라고 한다.

- 형식적으로, 위치 $h_0(x), h_1(x), h_2(x), \ldots$가 차례로 시도되며,

$$
h_i(x) = \big(hash(x) + f(i)\big) \bmod tableSize, \qquad f(0) = 0.
$$

- 함수 $f$가 *충돌 해결 전략*이다. 모든 데이터가 테이블 안에 들어가므로, 개별 체이닝보다 더 큰 테이블이 필요하다.

- 일반적으로 적재율은 $\lambda = 0.5$보다 낮아야 하며, 이러한 테이블을 [조사 해시 테이블]{.hl}이라 한다.
...
- [선형 조사]{.hl}에서 $f$는 $i$의 선형 함수이며, 보통 $f(i) = i$이다.
  - 이는 빈 위치를 찾아 위치를 순차적으로 시도하는 것과 같다.

- 오른쪽 테이블은 키 $\{89, 18, 49, 58, 69\}$를 삽입한 결과를 보여준다.

- 테이블이 비교적 비어 있어도, 원소가 채워진 위치의 군집이 형성된다.
  - 이것이 [1차 군집]{.hl}이다: 군집으로 해시되는 키는 여러 번 시도해야 하고 그런 다음 군집에 추가된다.
...
public:
    ...
    bool insert(const HashedObj& x) {
        ...
        if (++currentSize > array.size() / 2) rehash();   // 적재율 lambda < 0.5 유지
        return true;
    }
...
- [이차 조사]{.hl}는 선형 조사의 1차 군집을 없앤다.

- 그 충돌 함수는 이차식이며, 널리 쓰이는 선택은 $f(i) = i^2$이다.

- 1차 군집을 없애더라도, *같은* 위치로 해시되는 원소들은 같은 대체 위치를 조사한다.
  - 이를 [2차 군집]{.hl}이라 한다.
...
- 널리 쓰이는 선택은 $f(i) = i \cdot hash_2(x)$로, *두 번째* 해시 함수가 이동 간격을 결정한다.

- 오른쪽 테이블은 $\{89, 18, 49, 58, 69\}$를 삽입하면서 $hash_2(x) = x \bmod 9$를 사용한다.
  - $hash_2$는 절대 0이 되어서는 안 되며, $tableSize$보다 작은 소수 $R$에 대해 $R - (x \bmod R)$로 선택하면 잘 동작한다.
...
- 테이블이 너무 가득 차면 연산이 너무 오래 걸리고, 이차 조사 해결 방식에서는 삽입이 *실패*할 수도 있다.
  - 이는 많은 제거가 삽입과 뒤섞일 때도 발생할 수 있다.

- 해결책은 (새로운 해시 함수와 함께) 약 *두 배* 큰 새 테이블을 만들고, 원래 테이블 전체를 훑어, 삭제되지 않은 각 원소를 새로운 해시 값에 다시 삽입하는 것이다.

<div class="theorem-box">
<div class="theorem-box-title">예시</div>
<div class="theorem-box-body">

$h(x) = x \bmod 7$을 사용하는 크기 $7$의 선형 조사 테이블에 $13, 15, 24, 6$을 삽입한다. $23$을 삽입하면 테이블이 $70\%$ 넘게 차게 되어 크기 $17$의 새 테이블이 만들어진다 — $17$은 기존 크기의 두 배 이상에서 나타나는 첫 번째 소수이다.

</div>
</div>

- 이 전체 연산을 [재해싱]{.hl}이라 한다. 이는 비용이 크다: 수행 시간이 $\mathcal{O}(N)$이지만, 매우 드물게 일어나므로 *분할 상환* 비용은 작다.
...
- C++은 해시 테이블 자료구조를 구현하는 두 개의 표준 라이브러리 컨테이너를 제공한다: [`unordered_set`]{.hl}과 [`unordered_map`]{.hl}.

- (균형 이진 탐색 트리인) `set`이나 `map`과 달리, 이들은 원소를 특정한 순서로 유지하지 *않지만* 평균 $\mathcal{O}(1)$ 연산을 제공한다.
...
<div>
    ...
    <p style="margin: 0 0; font-size: 1.3rem;"><span style="color:#4a6fa5; font-weight:bold;">최악의 경우 O(1) 접근을 가지는 해시 테이블</span></p>
...
- 우리는 *최악의 경우*에도 $\mathcal{O}(1)$ 비용을 원한다.
  - 라우터의 하드웨어 조회 테이블이나 메모리 캐시 같은 응용에서, 탐색은 정해진 완료 시간을 가져야 한다.

- 단순화를 위해, $N$개의 원소를 모두 미리 알고 있다고 가정한다.

- 개별 체이닝 구현이 각 리스트가 최대 상수 개의 원소만 갖도록 보장할 수 있다면, 그것으로 충분하다.
  - 리스트를 더 많이 만들수록 리스트는 평균적으로 짧아진다 — 따라서 리스트가 충분히 많으면 충돌이 *전혀* 없기를 기대할 수도 있다.

---
layout: prism
heading: "완전 해싱 — 2단계 방식"
---
...
- 충돌이 없음을 보장할 만큼 리스트의 수 $M$을 충분히 크게 선택한다; 충돌이 감지되면 테이블을 비우고 다른 독립적인 해시 함수로 다시 시도한다.

- 이론적으로 $M$은 상당히 커야 한다 — 구체적으로 $M = \Omega(N^2)$ — 따라서 $N^2$개의 리스트를 사용하는 것은 실용적이지 않다.

- 실용적으로는, 각 *빈(bin)*에서 발생하는 충돌을 연결 리스트 대신 추가적인 해시 테이블을 사용해 해결한다.
  - 각 빈에는 원소가 몇 개뿐이므로, 빈에 대한 두 번째 해시 테이블은 빈 크기의 *제곱*으로 둘 수 있고, 높은 확률로 충돌이 없도록 유지된다.
...
- 첫 번째 해시 테이블(primary hash table)은 10개의 빈을 가진다:
  - 빈 1, 3, 5, 7은 비어 있다.
  - 빈 0, 4, 8은 원소가 하나씩 있다 — 위치가 하나인 두 번째 테이블.
  - 빈 2, 6은 원소가 둘씩 있다 — 위치가 넷인 두 번째 테이블.
  - 빈 9는 원소가 셋 있다 — 위치가 아홉인 두 번째 테이블.

- 각 두 번째 테이블은 충돌이 없어질 때까지 서로 다른 해시 함수로 구성된다. 이 방식이 [완전 해싱]{.hl}이다.
...
- [뻐꾸기 해싱]{.hl}에서는 $N$개의 원소, (각각 절반 이상 비어 있는) *두 개*의 테이블, 그리고 각 원소를 한 테이블의 한 위치에 대응시키는 두 개의 독립적인 해시 함수가 있다고 하자.
  - 불변식: 원소는 항상 자신의 두 위치 중 하나에 저장되므로, `find`는 오직 *두* 위치만 확인한다 — 최악의 경우 $\mathcal{O}(1)$.

- 알고리즘 자체는 단순하다:
  - 새 원소 $A$를 삽입하려면, 먼저 그것이 이미 있지 않은지 확인한다.
  - 첫 번째 해시 함수를 사용해, 그 첫 번째 테이블 위치가 비어 있으면 원소를 넣는다.
  - 이후의 원소 $B$가 이미 차 있는 위치로 가야 한다면, $B$가 *선제적으로* 그 자리를 차지하여 $A$를 밀어내고, $A$는 두 번째 해시 위치를 사용해 두 번째 테이블로 이동한다.

- 밀려난 원소는 다시 또 다른 원소를 밀어낼 수 있다 — 뻐꾸기 새끼가 다른 새끼들을 둥지 밖으로 밀어내는 것처럼.
...
- $A{:}(0,2)$, $B{:}(0,0)$, $C{:}(1,4)$, $D{:}(1,0)$, $E{:}(3,2)$, $F{:}(3,4)$를 삽입한 뒤 오른쪽 두 테이블을 얻는다 ($X{:}(y,z)$는 $X$의 두 해시 위치를 나타낸다).

- 이제 $G{:}(1,2)$를 삽입하면 원소들이 *끝없이* 밀려나고, 교체되고, 다시 밀려나게 된다 — [순환]{.hl}이다.

- 핵심 질문: 순환이 생길 확률은 얼마이며, 성공적인 삽입을 위한 밀어내기 횟수의 기댓값은 얼마인가?

- 적재율이 $0.5$ 미만이면 순환은 매우 드물다; $0.5$ 이상이면 훨씬 더 자주 일어나 뻐꾸기 해싱은 잘 동작하지 않는다.
...
int main() {
    ...
    for (int k : keys) {                      // 체인 앞쪽에 삽입
        auto& L = table[myhash(k)];
        L.push_front(k);
    }

    for (int i = 0; i < TS; i++) {            // 모든 슬롯의 체인 출력
        cout << i << ":";
...
- [홉스코치 해싱]{.hl}은 최대 조사 순열의 길이를 컴퓨터 구조에 맞게 최적화된 상수로 *제한*하여 고전적인 선형 조사를 개선한다.

- [보편 해싱]{.hl}은 해시 함수를 *무작위로* 선택하여, 원소들이 배열 슬롯에 균일하게 분산됨을 증명 가능하게 보장한다.

- [확장성 해싱]{.hl}은 탐색을 단 *두 번*의 디스크 접근만으로 수행할 수 있게 한다 — 데이터가 디스크에 있을 때 유용하다.

- 이러한 알고리즘들을 찾아 공부해 보시오.
...
- 코딩 테스트 문제 *세 개*를 선택해 풀어 보세요.
  - 웹사이트, 책 등에서 문제를 가져와도 됩니다.

- 각 문제를 여러분의 답과 함께 복사해 붙여넣으세요.
  - *스스로* 풀어 보세요; 문제가 너무 어렵다면 부정행위를 하지 말고 빈칸으로 남겨 두세요.
