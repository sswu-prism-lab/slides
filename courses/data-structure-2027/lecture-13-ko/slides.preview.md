...
- 어떤 원소가 삽입되었을 때, 이미 삽입된 원소와 같은 값으로 해싱된다면 [충돌]{.hl}이 발생하며 이를 해소해야 한다.

- 첫 번째 전략은 흔히 [개별 체이닝]{.hl}이라 불리며, 같은 값으로 해싱되는 모든 원소들의 리스트를 유지하는 방식이다.

- 해시 함수 $hash(x) = x \bmod 10$을 가정하면, 각 버킷은 그 버킷으로 매핑되는 키들의 연결 리스트를 담는다:
...
- [선형 조사]{.hl}에서 $f$는 $i$의 선형 함수이며, 보통 $f(i) = i$를 사용한다.
  - 이는 비어 있는 셀을 찾기 위해 셀들을 순차적으로 시도하는 것과 같다.

- 오른쪽 표는 키 $\{89, 18, 49, 58, 69\}$를 삽입한 결과를 보여준다.

- 테이블이 비교적 비어 있더라도, 원소가 채워진 셀들의 군집이 형성되기 시작한다.
  - 이러한 현상을 [1차 군집]{.hl}이라 하며, 군집으로 해싱되는 키는 여러 번의 시도가 필요하고 결국 군집을 더 키우게 된다.
...
- [이차 조사]{.hl}는 선형 조사의 1차 군집 문제를 없애는 충돌 해결 방식이다.

- 여기서 충돌 함수는 제곱식이며, 흔히 $f(i) = i^2$를 사용한다.

- 이차 조사가 1차 군집을 없애더라도, 같은 위치로 해싱되는 원소들은 동일한 대체 셀들을 조사하게 된다.
...
- 마지막으로 살펴볼 충돌 해결 방법은 [더블 해싱]{.hl}이다.

- 더블 해싱에서는 흔히 $f(i) = i \cdot hash_2(x)$를 선택한다.

- 오른쪽 표는 $hash_2(x) = x \bmod 9$를 사용한 더블 해싱 전략을 나타낸다.
...
- 지금까지 우리는 자료구조의 효율적인 *구현*에 관심을 두었다.
  - 어떤 알고리즘이 주어졌을 때, 실제 자료구조가 특정될 필요는 없다.
  - 적절한 자료구조를 선택하여 수행 시간을 최소화하는 것은 프로그래머의 몫이다.

- 이제 우리는 관심을 자료구조의 구현에서 [알고리즘의 설계]{.hl}로 옮긴다.
  - 문제를 푸는 데 사용되는 다섯 가지 일반적인 알고리즘 유형을 집중적으로 살펴본다.
  - 많은 문제에서 이들 중 적어도 하나의 방법은 잘 작동할 가능성이 높다.

- 각 알고리즘 유형에 대해 일반적인 접근법을 보고, 몇몇 예제를 살펴보며, 적절한 경우 시간·공간 복잡도를 논의할 것이다.
...
- 우리가 처음 살펴볼 알고리즘 유형은 [탐욕 알고리즘]{.hl}이다.
  - 다익스트라, 프림, 크루스칼 알고리즘은 탐욕적이며, 각 단계에서 미래의 결과를 고려하지 않고 지금 좋아 보이는 결정을 내린다.
  - 이는 어떤 *국소적 최적*이 선택될 수 있음을 의미한다.

- 알고리즘이 종료될 때, 우리는 이 국소적 최적이 [전역적 최적]{.hl}과 일치하기를 기대한다.
  - 그렇다면 이 알고리즘은 올바르며, 그렇지 않으면 *최적이 아닌* 해를 낸다.
  - 반드시 최상의 답이 요구되지 않을 때, 간단한 탐욕 알고리즘이 근사적인 답을 얻는 데 종종 사용된다.

- 가장 명확한 실생활 예시는 [거스름돈 문제]{.hl}이다.
  - 17,610원의 거스름돈을 주려면 10,000원 지폐 1장, 5,000원 지폐 1장, 1,000원 지폐 2장, 500원 동전 1개, 100원 동전 1개, 10원 동전 1개를 사용하며, 이는 지폐와 동전의 개수를 최소화함이 보장된다.

---
layout: prism
heading: "DIY: 탐욕적 거스름돈"
---
...
- 수행 시간 $t_1, t_2, \ldots, t_N$이 알려진 작업 $j_1, j_2, \ldots, j_N$이 주어졌다.

- 단일 프로세서와 *비선점형* 스케쥴링(한번 시작된 작업은 끝까지 수행됨) 상황에서, 평균 종료 시간을 최소화하는 스케쥴은 무엇일까?

- 간단한 선착순 방법은 평균 종료 시간 $25$를 준다.
...
- 스케쥴을 $j_{i_1}, j_{i_2}, \ldots, j_{i_N}$이라 하자. 첫 작업은 $t_{i_1}$에, 두 번째는 $t_{i_1}+t_{i_2}$에, 세 번째는 $t_{i_1}+t_{i_2}+t_{i_3}$에, 이런 식으로 종료된다.
...
총 비용에 영향을 주는 것은 두 번째 합뿐이므로, 짧은 작업을 먼저 스케쥴링하여 $C$를 최소화한다.
...
- 이 문제를 [여러 프로세서]{.hl}의 경우로 확장할 수 있다.

- 아홉 개의 작업 $j_1, \ldots, j_9$(수행 시간 $3, 5, 6, 10, 11, 14, 15, 18, 20$)와 세 개의 프로세서가 있을 때, 최적해는 다시 짧은 작업을 먼저 스케쥴링하며 프로세서에 라운드 로빈으로 분배한다.
...
- 대신 우리가 [마지막 작업이 끝나는 시점]{.hl}에만 관심이 있다고 가정하자.

- 같은 아홉 개 작업을 다르게 배치하면 최소 최종 종료 시간 $34$를 달성한다.
  - 이 스케쥴이 최소 *평균* 종료 시간을 갖지는 않지만, 전체 작업이 더 일찍 끝난다는 장점이 있다.

- *최소 평균 종료 시간*과 *최소 최종 종료 시간*은 매우 비슷해 보이지만, 이 새로운 문제는 첫 번째 문제보다 [훨씬 어렵다]{.hl}.
...
- 파일이 문자 `a`, `e`, `i`, `s`, `t`, 공백(`sp`), 개행(`nl`)만 포함한다고 하자.

- 고정 3비트 코드를 쓰면 파일은 $174$비트가 필요하다: $58$개 문자 $\times\ 3$비트.
...
- 일반적인 전략은 코드 길이를 [문자마다 다르게]{.hl} 하여, 자주 등장하는 문자가 짧은 코드를 갖도록 하는 것이다.
  - 모든 문자가 같은 빈도로 등장한다면 절감은 거의 없을 것이다.

- 코드를 [트라이]{.hl}로 표현하며, 문자는 리프에 위치하고 루트에서부터의 경로(왼쪽 $= 0$, 오른쪽 $= 1$)가 각 코드를 나타낸다.

- 원래의 고정 길이 전략은 깊이 $3$의 *완전하고 균형 잡힌* 트라이에 해당하며 — 모든 문자가 같은 깊이에 있어 절감이 없다.
...
- 문자를 [리프에만]{.hl} 배치하면, 어떤 비트 나열이든 항상 모호함 없이 해석할 수 있다.
  - 이러한 인코딩을 [접두사 코드]{.hl}라 하며, 어떤 코드도 다른 코드의 접두사가 되지 않는다.

- 드문 문자가 더 깊은 곳에 오도록 트라이를 재조정하면 비용 $173$비트의 코드를 얻을 수 있다.
  - 이는 고정 길이 $174$보다 약간 낫지만, 여전히 최적과는 거리가 있다.

- 따라서 문제는 모든 문자가 리프에 있는 *총 비용이 최소인 완전 이진 트라이*를 찾는 것이다.
...
- 오른쪽 코드는 단지 $146$비트만 필요하다.
  - 이러한 코딩 체계를 흔히 [허프만 코드]{.hl}라 부른다.

- 자주 등장하는 문자(`e`, `i`, `sp`)는 짧은 2비트 코드를, 드문 문자(`s`, `nl`)는 긴 5비트 코드를 받는다.
...
- 허프만 알고리즘은 가장 가벼운 두 트리를 반복적으로 병합하여 최적 트라이를 탐욕적으로 만든다:

<div class="sub-item-enum">

1. 트리들의 *포레스트*를 유지한다 — 처음에는 $C$개의 트리이며, $C$는 문자의 개수이다.
2. 트리의 [가중치]{.hl}는 리프들의 빈도 합이다.
3. $C-1$번에 걸쳐, 가중치가 가장 작은 두 트리 $T_1$과 $T_2$를 선택(동점이면 임의로)하여 $T_1$과 $T_2$를 서브트리로 갖는 새 트리를 만든다.

</div>

- 가중치를 키로 하는 우선순위 큐를 사용하면 각 병합은 $\mathcal{O}(\log C)$이 들어, 전체적으로 $\mathcal{O}(C \log C)$ 알고리즘이 된다.
...
int main() {
    ...
    while (pq.size() > 1) {                 // C-1번 병합
        ...
        int id = t.size();
        t.push_back({w1 + w2, -1, a, b});   // 새 내부 노드
        pq.push({w1 + w2, id});
    }
    // 트리를 순회하여 각 리프의 깊이(코드 길이)를 구함
...
- 알고리즘을 설계하는 또 다른 일반적인 기법은 [분할정복]{.hl}이다.
...
- 우리는 이미 몇몇 분할정복 알고리즘을 보았다 — 최대 부분수열 합 문제와 트리 순회 전략.
  - [병합정렬]{.hl}과 [퀵정렬]{.hl} 같은 정렬 알고리즘도 분할정복 알고리즘이다.

- 다음으로 이 패러다임의 또 다른 예시인 계산기하학 문제를 살펴본다.
...
- 어떤 분할정복 알고리즘이 각각 원래의 절반 크기인 두 문제를 처리하고 $\mathcal{O}(N)$의 추가 작업을 한다고 가정하자. 그러면 다음 점화식을 얻는다:
...
- 만약 $p_1 = (x_1, y_1)$이고 $p_2 = (x_2, y_2)$라면, 두 점 사이의 유클리드 거리는 $\big[(x_1 - x_2)^2 + (y_1 - y_2)^2\big]^{1/2}$이다.

- 우리는 [가장 가까운 점들의 쌍]{.hl}을 찾아야 한다.
  - 두 점이 같은 위치에 있다면 그 쌍이 거리 $0$으로 가장 가깝다.

- $N$개의 점이 있을 때 거리 쌍은 $N(N-1)/2$개이다.
  - 매우 짧은 프로그램으로 이들 모두를 확인할 수 있지만, 그 대가로 $\mathcal{O}(N^2)$ 알고리즘이 된다.
  - 이는 단지 전체 탐색이므로, 더 나은 방법을 기대할 수 있다.
...
- 점들이 $x$ 좌표로 정렬되었다고 가정하자.
  - 이는 최종 시간 한계에 $\mathcal{O}(N \log N)$을 더한다.

- 점 집합을 두 절반 $P_L$과 $P_R$로 나누는 가상의 수직선을 긋는다.
  - 점들이 $x$로 정렬되어 있으므로 이는 분명히 간단하다.
...
- 그렇다면 문제는 $d_C$를 계산하는 것이다. $\delta = \min(d_L, d_R)$라 하자.
  - 만약 $d_C$가 $\delta$를 개선한다면, $d_C$를 이루는 두 점은 분할선의 $\delta$ 범위 안에 있어야 하며 — 이 영역을 [스트립]{.hl}이라 한다.

- 스트립 안의 점들이 $y$ 좌표로 정렬되었다고 하자.
  - 만약 $p_i$와 $p_j$의 $y$ 좌표 차이가 $\delta$보다 크면, $p_{i+1}$로 넘어갈 수 있다.

- $p_i$의 $y$ 좌표로부터 $\delta$ 이내에 있는 점들만이 $\delta$를 이길 수 있으므로, 비교 횟수가 극적으로 줄어든다.
...
- 최악의 경우에도, 임의의 점 $p_i$에 대해 최대 [일곱]{.hl} 개의 점 $p_j$만 고려하면 된다.
  - 이 점들은 스트립의 왼쪽 절반에 있는 $\delta$×$\delta$ 정사각형이나 오른쪽 절반에 있는 정사각형 안에 놓여야 한다.
  - 각 $\delta$×$\delta$ 정사각형 안의 모든 점은 적어도 $\delta$만큼 떨어져 있다.

- 각 $p_i$마다 최대 일곱 점만 고려하므로, $\delta$보다 좋은 $d_C$를 계산하는 데 $\mathcal{O}(N)$이 든다.

- 따라서 우리는 $\mathcal{O}(N \log N)$ 해를 얻은 것처럼 보인다 — 절반 크기의 재귀 호출 두 번과 선형의 결합 작업.
  - 아직 *완전히* 얻은 것은 아니지만, 간단한 변형으로 이를 달성할 수 있다(매 단계마다 $\mathcal{O}(N \log N)$ 정렬을 피함).
...
- 재귀적인 어떤 수식도 바로 재귀적 알고리즘으로 옮길 수 있지만 — 컴파일러가 이를 제대로 살리지 못해 *비효율적인* 프로그램이 되는 경우가 많다.

- 이런 경우가 의심될 때, 우리는 재귀 알고리즘을 하위 문제의 답을 [표]{.hl}에 체계적으로 기록하는 [비재귀적]{.hl} 알고리즘으로 다시 작성하여 컴파일러를 돕는다.

- 이 접근법을 활용하는 한 기법이 [동적 프로그래밍]{.hl}으로 알려져 있다.
...
- [피보나치 수]{.hl}를 계산하는 자연스러운 재귀 프로그램은 매우 비효율적이다.
  - 같은 값을 반복해서 다시 계산하기 때문에 [지수]{.hl} 수행 시간이 필요하다.

- $N$번째 피보나치 수를 계산하려면 가장 최근에 계산된 두 값($F_{N-1}$과 $F_{N-2}$)만 있으면 된다.
...
long long calls = 0;                 // 재귀 호출 횟수를 셈
long long fib(int n) {               // 단순, 지수
    calls++;
    if (n <= 1) return n;
    return fib(n - 1) + fib(n - 2);
}
...
- [무작위 알고리즘]{.hl}에서는 알고리즘 수행 중 적어도 한 번 난수를 사용하여 결정을 내린다.
  - 수행 시간은 특정 입력뿐만 아니라 발생하는 난수에도 의존한다.

- 무작위 알고리즘의 최악 수행 시간은 흔히 무작위가 아닌 알고리즘의 최악 수행 시간과 같다.
  - 중요한 차이는, 좋은 무작위 알고리즘에는 *나쁜 입력이 없고* (입력에 대해) 나쁜 난수만 있다는 점이다.
...
- 이러한 알고리즘은 난수를 필요로 하므로, 이를 생성할 방법이 필요하다.
  - 컴퓨터에서 진정한 무작위성은 사실상 불가능한데, 생성된 수가 알고리즘에 의존하므로 정말로 무작위일 수 없기 때문이다.

- 일반적으로 단지 무작위처럼 보이는 [의사난수]{.hl}를 생성하는 것으로 충분하다.
  - 난수는 많은 알려진 통계적 성질을 가지며, 의사난수는 그중 대부분을 만족한다.

- $0$이나 $1$을 무작위로 생성하는 조잡한 방법 하나는 시스템 시계를 살펴보는 것이다.
  - 당연히 이 전략에는 많은 약점이 있다.

- 가장 간단한 실용적 방법은 [선형 합동 생성기]{.hl}로, 1951년 레머가 처음 기술하였다.
...
- 수 $x_1, x_2, \ldots$는 $x_{i+1} = A\,x_i \bmod M$을 만족하며 생성된다.

- [시드]{.hl}인 $x_0$의 어떤 값이 주어져야 한다.
  - 만약 $x_0 = 0$이면 수열은 무작위와 거리가 멀고, $A$와 $M$을 잘 선택하면 임의의 $1 \le x_0 < M$이 동등하게 유효하다.
  - 만약 $M$이 소수이면 $x_i$는 결코 $0$이 되지 않는다.

- $M = 11$, $A = 7$, $x_0 = 1$일 때: $7, 5, 2, 3, 10, 4, 6, 9, 8, 1, \ldots$ — 수열은 $M-1 = 10$개의 수 이후 반복된다.
...
- 레머는 [완전 주기]{.hl} 생성기를 위해 31비트 소수 $M = 2^{31} - 1 = 2147483647$과 $A = 48271$을 제안하였다.

- 이는 구현하기 간단해 보인다.
  - $x_i$를 $M$으로 나누면 $[0, 1)$ 범위의 무작위 실수를 생성할 수 있다.

- 최근 많은 라이브러리는 다음에 기반한 생성기를 사용한다
...
- 많은 경우에 백트래킹 알고리즘은 전체 탐색의 영리한 구현에 해당하며, 일반적으로 좋지 않은 성능을 보인다.
  - 그러나 항상 그런 것은 아니며, 그렇더라도 무차별 탐색에 비한 절감은 상당할 수 있다.

- 실용적인 예시는 새 집에 *가구를 배치하는* 문제이다.
  - 시도할 가능성은 많지만, 보통 실제로 고려되는 것은 소수뿐이다.
  - 배치의 바람직하지 않은 부분집합이 감지되기 때문에, 많은 나쁜 배치가 일찍 버려진다.
  - 한 단계에서 많은 가능성의 집합을 제거하는 것을 [가지치기]{.hl}라 한다.
...
- $x_i$를 $p_i$의 좌표라 하자. 이 $N$개의 점은 $|x_i - x_j|$ 형태의 (반드시 유일하지는 않은) 거리 $N(N-1)/2$개를 결정한다.

- [징수소 재건 문제]{.hl}는 거리들로부터 점 집합을 재건하는 것이다.
  - 이는 물리학, 분자생물학 등 여러 분야에서 응용된다.
  - 인수분해가 곱셈보다 어려워 보이듯, 재건은 건설보다 어려워 보인다.

- 다항 시간 내에 수행됨이 보장되는 알고리즘은 아직 아무도 찾지 못했다.
...
- $x_1 = 0$으로 시작한다. $10$이 $D$의 가장 큰 원소이므로 분명히 $x_6 = 10$이다.
  - $D$에서 $10$을 제거한다.

- 남은 가장 큰 거리는 $8$이므로, $x_2 = 2$이거나 $x_5 = 8$이다.
  - 대칭성에 의해 선택은 중요하지 않으므로 $x_5 = 8$로 둔다.
  - $D$에서 $x_6 - x_5 = 2$와 $x_5 - x_1 = 8$을 제거한다.

- 지금까지의 점: $\{0, 8, 10\}$, 그리고 $D = \{1, 2, 2, 3, 3, 3, 4, 5, 5, 5, 6, 7\}$.
...
- 이제 $7$이 $D$에서 가장 큰 값이므로, $x_4 = 7$이거나 $x_2 = 3$이다.
  ...
  - 판단할 근거가 없으므로, 하나를 시도해 보고 해에 이르는지 확인한다.

- 첫 번째 선택을 시도하여 $x_4 = 7$로 둔다. 이제 가장 큰 거리는 $6$이므로, $x_3 = 6$이거나 $x_2 = 4$이다.
  - 만약 $x_3 = 6$이면 $x_4 - x_3 = 1$인데, $1$이 더 이상 $D$에 없으므로 불가능하다.
  - 만약 $x_2 = 4$이면 $x_2 - x_1 = 4$와 $x_5 - x_2 = 4$인데, 역시 불가능하다.

- 이 추론은 해를 남기지 않으므로, 우리는 [백트래킹]{.hl}한다.
...
- $x_4 = 7$이 실패했으므로 $x_2 = 3$을 시도한다.
  - 이것마저 실패하면, 포기하고 해가 없음을 보고해야 한다.

- 다시 한번 $x_4 = 6$과 $x_3 = 4$ 사이에서 선택한다.
  - $x_3 = 4$는 불가능한 반면, $x_4 = 6$은 가능하다.

- 남은 유일한 선택은 $x_3 = 5$를 할당하는 것이며, 이로써 해를 얻는다: 점 집합은
$$
